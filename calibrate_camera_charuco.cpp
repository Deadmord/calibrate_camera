...
const char* keys  =
        ...
        "{dir      |       | Directory containing folders with images for each camera }";
}

int loopVideo(int argc, char* argv[], const char* about, const char* keys) {
    ...
    while (inputVideo.grab()) {
        ...
        std::vector<cv::Mat> images = loadImages(parser.get<std::string>("@images"));
...
int loopIDSimages(int argc, char* argv[], const char* about, const char* keys) {
    CommandLineParser parser(argc, argv, keys);
    parser.about(about);
    ...
    std::string directoryPath = parser.get<std::string>("dir");
    ...
    cv::glob(directoryPath + "/*", folders, true);

    for (const auto& folder : folders) {
        std::vector<cv::Mat> images = loadImages(folder);
        ...
        float squareLength = parser.get<float>("sl");
        float markerLength = parser.get<float>("ml");
        string outputFile = folder + "/calibration.yml";
        ...
        if (showChessboardCorners) {
            for (size_t frame = 0; frame < images.size(); frame++) {
                Mat imageCopy = images[frame].clone();
...
int main(int argc, char *argv[]) {
    if (loopIDSimages(argc, argv, about, keys) == 0)
    return 0;
}
